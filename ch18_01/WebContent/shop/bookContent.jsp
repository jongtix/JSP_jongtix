		...
		<table border="0" width="650" cellpadding="5" cellspacing="5"
			...
			<tr height="30">
				<td rowspan="6" width="150" align="center" valign="middle"><img
					src="${path}/imageFile/${book.book_image}" border="0" width="150"
					height="200"></td>
				<td width="500"><font size="+1"><b>${book.book_title}</b></font>
				</td>
			</tr>
			<tr>
				<td><b>저자:${book.author}</b></td>
			</tr>
			<tr>
				<td><b>출판사:${book.publishing_com}</b></td>
			</tr>
			<tr>
				<td><b>출판일:${book.publishing_date}</b></td>
			</tr>
			<tr>
				<c:if test="${book.discount_rate == 0}">
					<td>정가: <b><fmt:formatNumber value="${book.book_price}"
								pattern="###,##0" />원</b><br> 판매가: <b><fmt:formatNumber
								value="${book.book_price*(100-book.discount_rate)/100}"
								pattern="###,##0" />원 </b>
					</td>
				</c:if>
				<c:if test="${book.discount_rate != 0}">
					<td>정가: <s><fmt:formatNumber value="${book.book_price}"
								pattern="###,##0" />원</s><br> 판매가: <b><fmt:formatNumber
								value="${book.book_price*(100-book.discount_rate)/100}"
								pattern="###,##0" />원 </b>
					</td>
				</c:if>
				<td><b>재고수량:<input type="text" size="5" name="book_count"
						value="${book.book_count}" disabled>개
				</b><br> <b>수량:<input type="text" size="5" name="buy_count"
						value="1">개
				</b> <c:if test="${id=='not' }">
						<c:if test="${book.book_count==0 }">
							<b>일시품절</b>
						</c:if>
					</c:if> <c:if test="${id!='not'}">
						<c:if test="${book.book_count<=0}">
							<b>일시품절</b>
						</c:if>
						<c:if test="${book.book_count>0}">
					...
					onclick="location.href='shopMain.jsp'"></td>
			</tr>
			<tr>

...
public class BoardDao {
	...
	public int getTotal() {
		int total = 0;
		String sql = "select count(*) from pj_board where flag=2 and del != 'Y'";// 글번호
		try {
			conn = getConnection();
	...
	public Board getBoard(int num) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from (select rownum rn, a.* from (select * from pj_board where flag like '2%' and del != 'Y' order by num asc) a) where num = ?";
		Board board = new Board();
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);

...
public class Sub_boardDao {
	...
	public int insertSubboard(Sub_board sub_board) {
		...
		int number = 0;
		String sql1 = "select count(*)+1 from pj_sub_board";// 글번호
	...
	public List<Sub_board> getSubboard(int num) {
		...
		try {
			...
			while (rs.next()) {
				...
				sub_board.setReg_date(rs.getDate("reg_date"));
				sub_board.setDel(rs.getString("del"));
				list.add(sub_board);
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	public int deletesubBoard(int num) {
		...
		PreparedStatement pstmt = null;
		String sql = "update pj_sub_board set del = 'Y' where sub_num =?";
		try {
			  pstmt = conn.prepareStatement(sql);
			  pstmt.setInt(1, num);
			  result = pstmt.executeUpdate();
	...
	public List<Sub_board> selectList(int startRow, int endRow,int num) {
		...
		String sql = "select * from (select rownum rn, a.* from "
				+ "  (select * from pj_sub_board where ref=? order by sub_num )a)  where rn between ? and ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
